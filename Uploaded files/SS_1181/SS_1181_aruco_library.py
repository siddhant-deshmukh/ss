...
def mark_ArUco(img,Detected_ArUco_markers,ArUco_marker_angles):
    ...
    for markerID, markerCorner in Detected_ArUco_markers.items():
        ...
        bottomLeft = tbcm[2]
        bottomRight = tbcm[3]
        cv2.circle(img, topLeft, 5, color=(125,125,125), thickness=-1)
        cv2.circle(img, topRight, 5, color=(0,255,0), thickness=-1)
        cv2.circle(img, bottomLeft, 5, color=(255,255,255), thickness=-1)
        cv2.circle(img, bottomRight, 5, color=(180,105,255), thickness=-1)

        #center
        cXY = tbcm[4]
        cv2.circle(img, cXY, 5, (0, 0, 255), -1)

        #midpoint
        midPoint_tLtr = tbcm[5]
        cv2.line(img, midPoint_tLtr, cXY, (255, 0, 0), 4)

        # draw the ArUco marker ID on the image
        cv2.putText(img, str(markerID), (cXY[0] + 60, cXY[1]), font, 1, (0, 0, 255), 3)

        #angle
        ang = ArUco_marker_angles
        cv2.putText(img, str(ang[markerID]), (cXY[0] - 105, cXY[1]), font, 1, (0, 255,0), 2)
